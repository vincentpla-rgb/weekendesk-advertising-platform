...
| Tests unitarios | `src/pricing/__tests__/`, `lib/**/*.test.ts`, `app/**/*.test.ts` | Hecho — 267 tests + scripts/verify-rls-self-read.sh + scripts/verify-accepted-availability.sh + scripts/verify-pgcrypto-schema.sh + scripts/verify-proposals-list.sh + scripts/verify-proposal-numbering.sh + scripts/verify-volume-discount-toggle.sh + scripts/verify-media-fee-split.sh + scripts/verify-lead-time-force.sh (contra PostgreSQL 16 real) |
| Interfaz (Next.js) | `app/`, `lib/`, `components/` | Hecho — 4 pantallas del MVP + navegación y listados (ronda 7) + duplicar presupuesto y numeración (ronda 8) + reparto de medios forzable a mano (ronda 10) + antelación forzable a mano y checklist traducido (ronda 11) + vista previa del email sin enviar (ronda 12), interfaz interna en ES/FR/EN |
