...
5. Fechas de campaña inválidas: la fecha de fin no puede ser anterior a la de inicio (ronda 9) — solo se evalúa en modo "fechas concretas", con ambas fechas rellenas
6. Presupuesto de medios sin rellenar en una línea de media buy (ronda 9) — es una cifra de negociación con el cliente, nunca se calcula sola (§4.4)
7. Presupuesto de medios por debajo del fee mínimo de gestión (ronda 10, §4.4): el reparto automático dejaría el importe real al medio en negativo. Se resuelve forzando el reparto a mano o subiendo el presupuesto
8. Reparto de medios sin confirmar en un soporte de reparto siempre manual (INF-01, ronda 10, §4.4) — mientras no se fuerce a mano, el envío no puede salir con ese soporte
...
| Tests unitarios | `src/pricing/__tests__/`, `lib/**/*.test.ts`, `app/**/*.test.ts` | Hecho — 227 tests + scripts/verify-rls-self-read.sh + scripts/verify-accepted-availability.sh + scripts/verify-pgcrypto-schema.sh + scripts/verify-proposals-list.sh + scripts/verify-proposal-numbering.sh + scripts/verify-volume-discount-toggle.sh + scripts/verify-media-fee-split.sh (contra PostgreSQL 16 real) |
| Interfaz (Next.js) | `app/`, `lib/`, `components/` | Hecho — 4 pantallas del MVP + navegación y listados (ronda 7) + duplicar presupuesto y numeración (ronda 8) + reparto de medios forzable a mano (ronda 10), interfaz interna en ES/FR/EN |
