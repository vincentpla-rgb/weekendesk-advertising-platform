...
| **Comprobar `SUPABASE_SERVICE_ROLE_KEY` en Vercel** (Production, no solo Development/Preview): que sea de verdad la clave `service_role`, no la `anon` puesta por error | Vincent | Bloqueante si el login sigue devolviendo "no tiene acceso" con las filas correctas en la base de datos — verificado que el código ya usa esa clave correctamente (§10.3); si sigue fallando, la clave configurada es la sospechosa |
