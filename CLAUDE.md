...
| Tests unitarios | `src/pricing/__tests__/`, `lib/**/*.test.ts`, `app/**/*.test.ts` | Hecho — 176 tests + scripts/verify-rls-self-read.sh + scripts/verify-accepted-availability.sh + scripts/verify-pgcrypto-schema.sh + scripts/verify-proposals-list.sh (contra PostgreSQL 16 real) |
| Interfaz (Next.js) | `app/`, `lib/`, `components/` | Hecho — 4 pantallas del MVP + navegación y listados (ronda 7), interfaz interna en ES/FR/EN |
