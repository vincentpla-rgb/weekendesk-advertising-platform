...
| **Comprobar Site URL y Redirect URLs en el dashboard de Supabase** (deben incluir `/auth/callback`, no solo la raíz o `/proposals/new`) | Vincent | Bloqueante para el login en producción. Ver §10.3 |
| **Desactivar "Click Tracking" en Resend/SES para el SMTP de Supabase, o activar el "Send Email Hook"** (el enlace se está consumiendo antes de que el usuario lo abra, envuelto en `awstrack.me`) | Vincent | Bloqueante para el login en producción. Ver §10.3 |
...
| Login | `/login` | Magic link (Supabase Auth), destinado a salir por Resend vía el "Send Email Hook" (`app/api/auth/send-email/`) en vez del SMTP de pruebas de Supabase (límite de 4 correos/hora), pero **el hook no está activo en el proyecto real** (es una configuración del dashboard de Supabase, no de este repo — pendiente, ver §10.1.2): hoy Supabase manda su propio email con su plantilla por defecto, así que la vuelta real en producción es siempre `/auth/callback?code=...` (PKCE), no `/auth/confirm`. `emailRedirectTo` (`lib/supabase/login-redirect.ts`) apunta **siempre** a `/auth/callback?next=<destino>`, nunca directo a la página destino — apuntar directo fue un bug real en producción, ver §10.3. La lista blanca se comprueba en `lib/supabase/authorize-session.ts`, compartida por `/auth/callback` (activa) y `/auth/confirm` (solo si se activa el hook algún día), contra `allowed_emails` con la clave de servicio (`lib/supabase/team-access.ts`), que crea el `profiles` que falta en el primer login para evitar la dependencia circular con RLS (`is_team_member()` exige un `profiles` que aún no existe). `allowed_emails.full_name` es opcional: si no se rellena al dar de alta a alguien, se deriva del email. Un fallo al crear el `profiles` (constraint, RLS mal configurado, lo que sea) se registra explícitamente como fallo de aprovisionamiento y no como "no autorizado", para no repetir el mismo malentendido dos veces |
